...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    if (to.query.id) {
      this.query["g.parentId"] = to.query.id;
  ...
  created() {
    if (this.$route.query.id) {
      this.query["g.parentId"] = this.$route.query.id;
      this.onLoad(this.page);
    ...
    if (this.$route.query.noModel) {
      this.noModel = this.$route.query.noModel;
    }
  },
  methods: {
    goToPrice(item) {
    ...
    searchFather(label, item) {
      switch (label) {
        case "类别":
          this.query["a.typeId"] = item === -1 ? undefined : item.id;
          break;
        default:
          break;
      }
